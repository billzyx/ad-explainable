...
def train_a_round(args):
    ...
    if not args['train_attack']:
        train_dataset = dataloaders.load_train_dataset(
            args['train_dataset'], args['level_list'], args['punctuation_list'], args['train_filter_min_word_length'])
    else:
        train_dataset = dataloaders.load_train_attack_dataset(
            args['train_dataset'], args['model_description'], args['log_dir'], args['train_attack_step'],
            args['train_filter_min_word_length'], args['train_attack_reverse'])

    test_dataset = dataloaders.load_test_dataset(
        args['test_dataset'], args['level_list'], args['punctuation_list'], args['test_filter_min_word_length'])
...
def main():
    ...
    ap.add_argument(
        "--batch_size", required=False, type=int,
        default=8,
    )
    ap.add_argument(
        "--learning_rate", required=False, type=float,
        default=0.00001,
    )
    ap.add_argument(
        "--decay", required=False, type=float,
        default=1.0,
    )
    ap.add_argument(
        "--max_epochs", required=False, type=int,
        default=100,
    )
    ap.add_argument(
        "--level_list", required=False, nargs="+", type=int,
        default=(15, 94),
    )
    ap.add_argument(
        "--punctuation_list", required=False, nargs="+",
        default=(',', '.',),
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        default='ADReSS20-train',
    )
    ap.add_argument(
        "--test_dataset", required=False,
        default='ADReSS20-test',
    )
    ap.add_argument(
        "--train_rounds", required=False, type=int,
        default=10,
    )
    ap.add_argument(
    ...
    ap.add_argument(
        "--train_attack", required=False, action='store_true',
        default=False,
    )
    ap.add_argument(
        "--train_attack_reverse", required=False, action='store_true',
        default=False,
    )
    ap.add_argument(
        "--train_attack_step", required=False, type=int,
        default=1,
    )
    ap.add_argument(
        "--train_filter_min_word_length", required=False, type=int,
        default=20,
    )
    ap.add_argument(
        "--test_filter_min_word_length", required=False, type=int,
        default=0,
    )
    ap.add_argument(
        "--log_dir", required=False, default='log_20',
    ...
    if args['model_description'] is not None:
        if not args['train_attack']:
            args['log_dir_final'] = os.path.join(os.path.abspath(args['log_dir']), args['model_description'])
        else:
            if args['train_attack_reverse']:
                args['log_dir_final'] = os.path.join(
                    os.path.abspath(args['log_dir']),
                    args['model_description'] + '-attack_reverse-' + str(args['train_attack_step']))
            else:
                args['log_dir_final'] = os.path.join(
                    os.path.abspath(args['log_dir']),
                    args['model_description'] + '-attack-' + str(args['train_attack_step']))
    else:
        args['log_dir_final'] = os.path.join(os.path.abspath(args['log_dir']), str(datetime.now()).replace(' ', '_'))
    os.makedirs(args['log_dir_final'])

    model_dir_path = os.path.join(args['log_dir_final'], 'models')
    os.makedirs(model_dir_path)

    training_log_dir_path = os.path.join(args['log_dir_final'], 'training_logs')
    ...
    with open(os.path.join(os.path.abspath(args['log_dir_final']), 'args.json'), 'w') as f:
